...
- Нативные Windows, macOS и Linux/X11 adapters могут кратко показывать рядом с курсором новые нажатия клавиатуры и кнопок мыши (`Ctrl + C`, `LMB`). Функция отключена по умолчанию, поскольку записывает чувствительный ввод; включение доступно отдельным Video switch, CLI-флагом `--show-input` или `video.showInputOverlay=true` в profile JSON и применяется также к replay buffer.
...
- На Windows, macOS и Linux/X11 системное меню позволяет явно включить hotkeys, а кнопка с клавиатурой в header открывает их настройку: для выбора области (`Ctrl+Shift+F8`), запуска/остановки записи (`Ctrl+Shift+F9`), pause/resume (`Ctrl+Shift+F10`) и сохранения активного replay buffer (`Ctrl+Shift+F11`) можно выбрать уникальные сочетания поддерживаемых клавиш и модификаторов. Кнопка сброса возвращает сочетания по умолчанию. Регистрация не запускает capture и полностью снимается при выключении или закрытии приложения; Wayland не подменяется XWayland-регистрацией.
