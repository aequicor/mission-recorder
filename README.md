...
> Текущий релиз: [v0.1.5](https://github.com/aequicor/mission-recorder/releases/tag/v0.1.5) — контекстные запросы разрешений macOS, брендированный DMG и стабильные пути сохранения относительно домашнего каталога. Проект остаётся pre-1.0 и активно развивается.

## Установка

Готовые пакеты публикуются в [GitHub Releases](https://github.com/aequicor/mission-recorder/releases/latest):

| Платформа | Артефакт | Поддержка в v0.1.5 |
| --- | --- | --- |
| Windows x64 | `-setup.exe` | Основная desktop-платформа |
...
Desktop-пакеты содержат собственный Java runtime. Пакеты v0.1.5 пока не подписаны и не notarized, поэтому ОС может показать предупреждение издателя. Перед запуском сверяйте файл с `SHA256SUMS.txt` из того же релиза. История изменений находится в [CHANGELOG.md](CHANGELOG.md), а процесс публикации — в [docs/releasing.md](docs/releasing.md).

Единый Windows `-setup.exe` устанавливает приложение в профиль текущего пользователя без запроса прав администратора и не пытается обновить или удалить старые MSI-установки. Выбор системного каталога установки намеренно отключён.
...
- GUI выполняет permission preflight перед каждой записью/replay: незаявленный статус считается отказом, а capture adapters и output не открываются до успешной проверки. На macOS автоматический preview не показывает системный prompt: контекстный запрос появляется только после явного действия пользователя, а отказ ведёт в нужную панель System Settings с ручной повторной проверкой. Платформенная семантика описана в [docs/permissions.md](docs/permissions.md).
...
- Добавлен `:capture-macos-coregraphics`: CoreGraphics discovery и window/application capture с рендерингом `CGImage` в контролируемый Retina RGBA context. Native gateway разделяет тихий `CGPreflightScreenCaptureAccess` и явный `CGRequestScreenCaptureAccess`, проверяет AVFoundation TCC status и открывает целевые privacy panes после отказа; app bundle содержит конкретные Screen Capture и Microphone usage descriptions.
...
- В Output доступен отдельный редактор каталога и `.mp4`-шаблона имени с подстановкой `{timestamp}`; Apply обновляет предпросмотр следующего пути и атомарно сохраняет policy в локальных настройках. Относительный каталог разрешается от домашнего каталога пользователя, поэтому профиль по умолчанию `recordings` пишет в `~/recordings` независимо от рабочего каталога установленного приложения; абсолютный путь используется без изменения.
