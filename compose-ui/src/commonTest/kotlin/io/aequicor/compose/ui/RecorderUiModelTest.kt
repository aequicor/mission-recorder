...
class RecorderUiModelTest {
    ...
    fun startRequiresSourceOutputAndIdleRecorder() {
        ...
        assertTrue(ready.canTakeScreenshot)
        assertTrue(ready.canTakeScreenScreenshot)
        assertFalse(ready.copy(status = RecorderStatus.Recording).canTakeScreenshot)
        assertFalse(ready.copy(status = RecorderStatus.Recording).canTakeScreenScreenshot)
