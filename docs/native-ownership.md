...
Window capture через `PrintWindow` выделяет DC и bitmap на один кадр; любая ошибка после частичного выделения проходит через `finally`, который восстанавливает selection и освобождает созданные объекты. GDI screen fallback удерживает свои DC и bitmap до `close()`. Оба GDI-пути используют только `SRCCOPY`: `CAPTUREBLT` намеренно не применяется, потому что при частой смене формы аппаратного указателя он может вызывать мерцание системного курсора. Cursor и opt-in input overlays опрашивают только текущие позицию указателя и состояния клавиш/кнопок, затем рисуют собственные RGBA-образы; глобальные hooks и дополнительные Win32 cursor handles не удерживаются.
